    ...
    >
      ...
      >
        ...
        <div v-if="!page.pageBack && !page.pageFront" class="programme">
          <div class="programme-title">节目介绍</div>
          <div v-for="(para, idx) in programme" :key="idx" class="programme-content">{{ para }}</div>
        </div>
      </div>
      <div
  ...
  methods: {
    ...
    setPageFrontBackground(index) {
      if (index === 0) {
        return {
          backgroundImage:
            "url('/GoBack/images/m_album_cover.webp')",
    ...
    changePage() {
      ...
      if (this.currentIndex >= pageDoms.length) {
        ...
        while (index < pageDoms.length) {
          ...
          pageDoms[index].style.transform = 'rotateX(0)'
    ...
    //   book.style.transform = 'translateY(50%)'
      ...
      pageDoms[this.currentIndex].style.transform = 'rotateX(150deg)'
    ...
    prevPage() {
      ...
      this.currentIndex--
      pageDoms[this.currentIndex].style.transition = '2s'
      pageDoms[this.currentIndex].style.transform = 'rotateX(0)'
